...
class Login extends React.Component {
    ...
    makeQuery = () => {
        ...
        query.append('redirect_uri', URLS.GOOGLE_REDIRECT_URL);
        query.append('state', state);
        query.append('scope', KEYS.SCOPE);
        query.append('access_type', 'offline');
        query.append('response_type', 'code');

        this.setState({ url: `${URLS.GOOGLE_AUTHORIZE}${query}` });
    }
    ...
    signinUser = () => {
        ...
        if (state && state === localState) {
            Api.BackendServer.post('login/social/token_user/google-oauth2/', {
                code,
                client_id: KEYS.CLIENT_ID,
                redirect_uri: URLS.GOOGLE_REDIRECT_URL,
            }).then(response => {
    ...
    render() {
        return (
            <Page style={{ flex: 1, display: 'flex', height: '100vh', width: '100vw', padding: 0, margin: 0, alignItems: 'center', justifyContent: 'center' }} loading={this.state.loading}>
                <Button onClick={() => this.setState({ loading: true })} href={this.state.url} type="primary" icon="google" size={'large'}>Entrar com o Google</Button>
            </Page>
        );
    }
