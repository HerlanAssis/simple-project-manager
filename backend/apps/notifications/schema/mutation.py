...
class CreateWatcherAsGuest(graphene.Mutation):
  ...
  def mutate(root, info, invitation_code, input=None):
    ok = False
    vigilant_instance = Watcher.getVigilantBy(invitation_code=invitation_code)
...
class CreateWatcherAsContributor(graphene.Mutation):
  ...
  @staticmethod
  def mutate(root, info, project_id, input=None):
    ok = False

    # verificar se ele é um contribuidor
    try:
      access_token = info.context.user.social_auth.get(provider='github').extra_data['access_token']
